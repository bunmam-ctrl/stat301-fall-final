...
```{r}
#| label: (fig 4a) The accessibility of abortion in Western Asia and Africa
...
```{r}
#| label: (fig 4c) the accessibility of abortion in Latin America
# Graph Latin America
...
```{r}
#| label: (Graph 5) Justification for Violence across subregions - Data preparation
#reorder the factor in descending order (justify_violence)
justify_totals <- violence_tidy|>
  group_by(justify_violence)|>
  summarise(total_value = sum(value_agree, na.rm = TRUE))|>
...
```{r}
#| label: tidy_china_marriage_divorce- add urban and rural area
...
```
```{r}
#| label: tbl-1-china-global-gender-indices
#| tbl-cap: "China's Performance in Global Development and Gender Equality Indice in 2021"
