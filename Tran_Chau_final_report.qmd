...
```{r}
#| label: tidy_violence - change and reoder factor justify_violence, subregion 
...
```
```{r}
#| label: Top 5 global gender inequality index (table 1)
...
```{r}
#| label: (Graph 3) Gender inequality over time
#| fig_cap: "Gender Inequality Index Over Time"
...
```{r}
#| label:  (Graph 4a) The accessibility of abortion in Western Asia and Africa
...
```{r}
#| label: (Graph 4b) The accessibility of abortion in Eastern and Southeastern Asia
...
```{r}
#| label: (Graph 4c) the accessibility of abortion in Latin America
# Graph Latin America
...
```{r}
#| label:  (Graph 4d) The accessibility of abortion in Europe
...
```{r}
#| label: Justification for Violence across subregions (Graph 4, Data preparation)
#reorder the factor in descending order (justify_violence)
justify_totals <- violence_tidy|>
  group_by(justify_violence)|>
  summarise(total_value = sum(value_agree, na.rm = TRUE))|>
...
```{r}
#| label: Justification for Violence across subregions (Graph 5)
...
violence_tidy|>
  ...
  labs(
    ...
    subtitle = "Analysis of public opinion on different causes of domestic violence \n across developing countries in 2020",
    x = NULL,
...
```
```{r}
#| label: (Graph 6) linegraph male vs female 
#| fig-cap: "Gender Proportion Trends in China over 1950-2021"
...
#graph
china_population_gender_graph <- china_population_gender|>
...
```{r}
#| label: (Graph 7) linegraph rural vs urban 
#| fig-cap: "Population Distribution Changes in China over 1950-2021"
...
china_population_area|>
  ...
  labs(
    ...
    subtitle  = "Analysis of Urban and Rural Population Trends Over 1950-2021",
    x = "Year",
    y = "Proportion",
    color = "Area type"
  )+
